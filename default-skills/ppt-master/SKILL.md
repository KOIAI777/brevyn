...
**MUST run before any PPT workflow route, including the main pipeline and standalone workflows:**

```bash
python3 ${SKILL_DIR}/scripts/preflight.py --json
```

- If `status` is `ok`, continue silently.
- If `status` is `warning`, continue. Mention optional missing features only when the current request needs them.
- If `status` is `failed`, stop immediately. Do not generate PPTX, do not write custom fallback PPT scripts, and do not continue the workflow. Explain in the user's language that the PPT runtime is not ready, include the current Python version, list the missing required packages or too-old Python version, and show the exact `installCommand` from the preflight output.
- Do not replace this workflow with a hand-written `python-pptx` script when preflight fails. The user must repair the runtime first.

## First-Turn Source Asset Inventory Gate

**MUST run before the first user-visible PPT plan whenever the source is a report, paper, coursework document, existing deck, spreadsheet, PDF, DOCX, PPTX, image, screenshot, or mixed source package.**

The first visible plan must be based on both the source's text/storyline and its reusable visual/data assets. Plain text extraction alone is not a complete source read when the file type may contain images, charts, tables, screenshots, formulas, or statistical outputs.

Before saying there are no reusable assets, inspect the source using the best available route:

| Source type | Required inventory pass |
|---|---|
| DOCX | Inspect OOXML package contents for native tables and `word/media` embedded images, or use the DOCX/default document skill. |
| PDF | Extract text and inspect/render candidate pages that may contain figures, charts, tables, scanned pages, or screenshots. |
| PPTX | Use `source_to_md.py` plus `project_manager.py import-sources` so `pptx_intake.py` writes `analysis/source_profile.json` and per-deck identity/slide-library files. |
| XLSX / CSV / TSV | Inspect sheet/table structure, headers, candidate numeric ranges, and any embedded charts/images when available. |
| Images / screenshots | Identify whether the image is a chart, table, SPSS/Excel output, document page, photo, diagram, or figure panel before planning. |
| Mixed folders/packages | Inventory each source class separately and merge the asset list. |

If only text has been read, say `素材盘点尚不完整` and list what still needs checking. Do not claim that no reusable figures/tables/charts exist until the relevant visual/table/chart inventory has actually been attempted or the format clearly cannot contain them.

The first visible PPT plan must include a compact asset section with these buckets:

1. `已确认素材` — assets actually found and inspected.
2. `疑似/需进一步提取素材` — assets suggested by captions, filenames, page structure, sheet names, or conversion metadata but not fully inspected.
3. `未能检查的素材` — source regions or file types that could not be inspected with the current runtime/tools.

For each important asset, state the intended treatment:

| Treatment | Meaning |
|---|---|
| `原图复用` | Reuse the source visual as an image. |
| `局部裁剪` | Crop/split a dense figure, table, screenshot, or panel. |
| `可编辑表格` | Recreate as an editable table only when values/labels are explicit. |
| `可编辑图表` | Recreate as an editable chart only when values/labels/units are explicit and recreation improves readability. |
| `简化示意图` | Convert a workflow, model, or concept into a simplified diagram without inventing facts. |
| `文字结论` | Use as slide takeaway or speaker-note support without visual insertion. |
| `跳过` | Omit because it is low-value, duplicated, unreadable, or unsafe to reproduce. |

Prefer user-provided report visuals, screenshots, charts, and tables as the starting point. Recreate or redraw only when exact values are available and the new visual will be clearer without changing labels, units, category order, statistical notation, or meaning. Do not invent missing chart values or replace a curated report visual with a new chart from memory.
...
After Step 7.3 succeeds, the assistant's final user-facing message MUST use this stable structure and MUST NOT invent a different delivery layout:

```markdown
已生成 PPT：
<absolute_or_project_relative_path_to_exports/main.pptx>

项目目录：
<project_path>

可继续编辑：
- 源 SVG：<project_path>/svg_output/
- 素材：<project_path>/images/
- 备注：<project_path>/notes/

可选输出：
- SVG 预览版：<path_to_svg_snapshot.pptx>（仅当本次实际生成）
- 原生图表/表格版：<path_to_native_charts.pptx>（仅当本次实际生成）
```

Rules:
- The canonical deliverable is always the main native PPTX in `exports/<project_name>_<timestamp>.pptx`.
- Report only files that actually exist. Do not mention optional outputs unless they were produced in this run.
- Do not present `svg_output/`, `svg_final/`, `analysis/`, `backup/`, or `sources/` as final deliverables. They are internal project artifacts; mention only `svg_output/`, `images/`, and `notes/` under "可继续编辑".
- If export fails, do not use the final output structure. Report the failed Step 7 sub-step, the exact error, and the artifact that must be fixed before retrying.
